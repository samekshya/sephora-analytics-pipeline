...
|---|---|
| Current branch | `main` |
| `main` | merged, pushed, and in sync with `origin/main` |
| Remote state | up to date — everything through the presentation work is pushed |
| Deck | **`presentation/sephora_pipeline_deck.html` — eleven slides, current, the one to present.** Opens in a browser, no build step. Embeds the generated SVG diagrams, the DAG graph, a generated trend figure and two cropped dashboard charts |
...
presentation/       sephora_pipeline_deck.html — the 11-slide deck; assets/, build_charts.py, speaker_notes.md
...
# 3. Tests
py -m pytest              # 52 collected; 1 skips locally (airflow not in host venv)
...
1. Optional: re-capture the four original screenshots (see `docs/screenshots/README.md`).
...
2. Optional: `docs/07_dashboard_insights.md` predates the review-length view, and
   `docs/03_architecture.md` predates the generated architecture diagram.
3. If the numbers ever move, regenerate the PDF guide — `docs/guide/README.md` has the
...
1. **Q1** — Which brands and categories earn the highest ratings, and which underperform?
2. **Q2** — Hype vs reality: which products have high `loves_count` but low ratings?
3. **Q3** — Does price predict satisfaction — do expensive products actually rate better?
4. **Q4** — Do reviewers with different skin types and tones rate the same products differently?
5. **Q5** — How do review volume and average rating trend over time?

> Q2 was added during the phase-9 remediation and is backed by `vw_hype_vs_reality`.
>
...
| — Presentation deck | **Done** — `presentation/sephora_pipeline_deck.html`, 11 slides timed to 8:00, every slide rendered and visually inspected. Covers all five course requirements including the KPI strip and trend (slide 9). Supersedes the 8-slide PowerPoint, which is kept and marked stale |
...
| **Q3 — price vs satisfaction is an inverted U** | Under $15 **4.2383** → $15-30 4.2756 → $30-50 4.3055 → **$50-100 4.3335 (peak)** → $100+ **4.2708 (falls back)**. Rating spread turns in the SAME band: stddev 1.2211 → 1.1861 → 1.1498 → **1.0996 ($50-100, tightest)** → 1.1366 ($100+, widens again). $50-100 is the sweet spot on both measures. **Not** a monotone fall — an earlier note here and in docs/07 and README claimed it was, contradicting their own tables |
| Q1a — best brands (≥500 reviews) | MARA 4.8608 · DAMDAM 4.7394 · Dr. Lara Devgan 4.7164 |
| Q1a — worst brands (≥500 reviews) | Topicals 3.6590 · DERMAFLASH 3.7856 · Isle of Paradise 3.8601 |
| Q1b — categories (secondary, see D16) | By volume: Moisturizers 297,201 (4.3172) · Treatments 221,871 (4.3040) · Cleansers 200,477 (**4.3443, best**) · Mini Size 85,433 (4.2856) · Eye Care 74,966 (**4.1784**) · Masks 70,483 (4.3410) · Lip Balms 61,321 (4.3327) · Sunscreen 41,126 (**4.1665, worst**) |
| **Q2 — hype vs reality** | Most overhyped: The Ordinary Vitamin C 23% — 132,601 loves, **3.4456** rating. The INKEY List Oat Cleansing Balm 127,819 loves / 3.6044. Sleeper hits: MACRENE actives products, ~200 loves and **4.93** ratings |
| Q2 — trend | Volume grew 2,760 (2008) → 215,278 (2020), then eased. Rating dipped to **4.2075 in 2020** and recovered to 4.3384 by 2022 |
| Q4 — skin type | Combination 4.3092 → dry 4.2911 → normal 4.2822 → **oily 4.2708**. Real but small spread (0.038) — worth stating as a weak signal, not a headline |
